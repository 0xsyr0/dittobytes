...
int main(int argc, char** argv) {
    ...
    // Get file size
     if (fstat(hShellcodeFile, &sFileStat) < 0) {
        ...
        PRINT_SUCCESS("Obtained shellcode file size (in bytes): %lu.", iShellcodeSize);
    }

    // Check file size
    if (iShellcodeSize <= 0) {
        PRINT_FAILURE("Shellcode file is empty or has an invalid size (%lu bytes).", iShellcodeSize);
        iResult = EXIT_FAILURE;
        goto CLEANUP_AND_RETURN;
    } else {
