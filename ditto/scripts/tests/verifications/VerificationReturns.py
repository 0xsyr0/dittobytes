...
class VerificationReturns:
    ...
    def verify(self, feature_test_specification):
        ...
        try:
            ...
            file_type_filter = 'all' if len(feature_test_specification['test_arguments']) != 3 else feature_test_specification['test_arguments'][2]

            # RAW format
            if file_type_filter in ['all', 'raw']:
                ...
                shellcode_result = subprocess.run(command, capture_output=True, text=True, timeout=10)

                if shellcode_result.returncode != 0:
                    ...
                    result_return_value_matches = re.findall(result_return_value_pattern, shellcode_result.stdout)
                    raw_return_value = result_return_value_matches[0][1]
                    raw_return_value_is_valid = str(raw_return_value) == str(expected_return_value)
            else:
                raw_return_value_is_valid = True

            # EXE format
            if file_type_filter in ['all', 'exe']:
                ...
                # Only signed integers up to 32-bit are supported in EXE exit codes
                if expected_return_type in ['int8_t', 'uint8_t', 'int16_t', 'uint16_t', 'int32_t']:
