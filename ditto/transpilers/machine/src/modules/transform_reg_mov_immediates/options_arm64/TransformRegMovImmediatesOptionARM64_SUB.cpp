 ...
 */
class TransformRegMovImmediatesOptionARM64_SUB {
...
public:
    ...
    /**
     * Main execution method for the TransformRegMovImmediatesOptionARM64_SUB class.
     *
    ...
    bool runOnMachineFunction(MachineFunction &MF, bool modifyAll) {
        ...
        dbgs() << "        ↳ Running ARM64 module: TransformRegMovImmediates(option=SUB,modifyAll=" << modifyAll << ").\n";
...
private:
    ...
    size_t getMovImmediateSize(const MachineInstr &instruction) {
        ...
        switch (opcode) {
            ...
            case AArch64::MOVi64imm:
                ...
                break;
            default:
                report_fatal_error(formatv("TransformRegMovImmediatesOptionARM64_SUB - Unknown immediate size for opcode {0:X}: {1}.", opcode, instruction));
                return 0;
        }
    }
    ...
    unsigned getMovSizeSubReplacement(const MachineInstr &instruction) {
        ...
        switch (opcode) {
            ...
            default:
                report_fatal_error(formatv("TransformRegMovImmediatesOptionARM64_SUB - Unknown SUB replacement size for opcode {0:X}: {1}.", opcode, instruction));
                return 0;
        }
    }
